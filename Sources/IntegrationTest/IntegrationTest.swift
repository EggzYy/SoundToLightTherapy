...
// Note: This test file should not be included in iOS builds
...
actor IntegrationTest {
    ...
    func testReducedMotionSupport() async {
        ...
        if observedChanges > 0 {
            ...
            print(
                "Final reduced motion state: \(await ReducedMotionSupport.isReducedMotionEnabled)")
        } else {
    ...
    func testColorContrastCompliance() async {
        ...
        let therapyViewCombinations:
            [(
                ...
                (
                    foreground: ColorContrastSupport.AccessiblePalettes
                        .primaryGreenForLightBackground,
        ...
        for combination in allCombinations {
            ...
            if !result.isCompliant {
                allCompliant = false
                print(
                    "❌ FAIL: \(String(format: "%.2f", result.ratio)):1 ratio for foreground RGB(\(combination.foreground.red), \(combination.foreground.green), \(combination.foreground.blue)) on background RGB(\(combination.background.red), \(combination.background.green), \(combination.background.blue))"
                )
            }
        }
